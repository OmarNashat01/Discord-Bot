...
bot = commands.Bot(command_prefix="botaya ", intents=intents)
...
async def jimmy(ctx):
    ...
    if user.voice != None:
        ...
        if vc.is_playing():
            vc.stop()
        source = await nextcord.FFmpegOpusAudio.from_probe("jimmy1.mp4", method='fallback')
        vc.play(source)
...
async def dola3(ctx):
    ...
    if user.voice != None:
        ...
        if vc.is_playing():
            vc.stop()
        source = await nextcord.FFmpegOpusAudio.from_probe("jimmy_dola3.mp4", method='fallback')
        vc.play(source)
...
# @commands.has_permissions(ban_members=True)
async def seyah(ctx, user: Member, reason=None):

    embed = Embed(title="successfully timed out user", color=nextcord.Color.red())
    if user.voice != None:
        try:
            vc = await user.voice.channel.connect()
        except:
...
@commands.has_permissions(ban_members=True)
async def Timeout(ctx, user: Member, time=None, *args):
    if args:
        args = ''.join(args)
    if time == None:
        time = 30
    await user.timeout(datetime.timedelta(seconds=int(time)))
    # await ctx.message.author.guild.timeout(user, reason=reason)
    embed = Embed(title="successfully timed out user", color=nextcord.Color.red())
    embedData = {
		"Member": user.mention,
		"Reason for ban": reason,
